    ...
    const sendPost = (e) => {
        ...
        db.collection('posts').add({
            ...
            if(imageToPost){
                ...
                removeImage();
                uploadTask.on('state_change',null,erorr => console.log(error),
        ...
        <div className="p-2 mt-6 font-medium text-gray-500 bg-white shadow-md rounded-2xl">
            <div className="flex items-center p-4 space-x-4">
                ...
                <form className="flex flex-1">
                    <input className="flex-grow h-12 px-5 bg-gray-100 rounded-full flex-group focus:outline-none" 
                            ref={inputRef}
                        type="text" placeholder={`What's on your mind ${session.user.name}`}/>
                    <button hidden type="submit" onClick={sendPost}>Submit</button>
                </form>
                {imageToPost && (
                    <div onClick={removeImage} className="flex flex-col transition duration-150 transform cursor-pointer hover:brightness-110 hover:scale-105"> 
            ...
            <div className="flex p-3 border-t justify-evenly">
                ...
                <div className="inputIcon">
                    <VideoCameraIcon className="text-yellow-300 h-7" />
